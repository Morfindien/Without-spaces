...
def main() -> int:
    ...
    try:
        ...
        spt_candl_data = module_info("spt_candl_data")
        act = module_info("act_dr6_cmbonly")
        result["checks"]["classy"] = classy
        result["checks"]["candl"] = candl
        result["checks"]["spt_candl_data"] = spt_candl_data
        ...
        # Verify the official frozen SPT package points into the frozen checkout.
        spt_data_file = Path(spt_candl_data["file"]).resolve()
        spt_root = REPOS["spt"].resolve()
        try:
            spt_data_file.relative_to(spt_root)
        except ValueError:
            raise RuntimeError(
                "spt_candl_data is importable but is not loaded from the frozen "
                f"SPT checkout. Loaded: {spt_data_file}; expected under {spt_root}"
            )
